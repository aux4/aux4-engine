...
class CommandParametersFactory {
  ...
  async getParameter(target, name) {
    for (const retriever of this.parameterRetrivers) {
      const value = await retriever.lookup(target.command, target.params, name);
      if (value !== undefined) {
        return value;
      }
    }
  ...
  create(command, params) {
    ...
    return new Proxy(commandParameters, {
      get(target, name, receiver) {
        ...
        return new Promise(resolve =>
          self.getParameter(target, name).then(value => {
