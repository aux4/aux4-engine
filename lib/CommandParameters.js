...
class CommandParametersFactory {
  ...
  async getParameter(target, name, args, proxy) {
    for (const retriever of this.parameterRetrivers) {
      const value = await retriever.lookup(target.$command, target.$params, name, args, proxy);
      if (value !== undefined) {
        return value;
      }
    }
    ...
    const proxy = new Proxy(commandParameters, {
      get(target, name, receiver) {
        ...
        return new Promise(resolve =>
          self.getParameter(target, name, args, ref.proxy).then(value => {
